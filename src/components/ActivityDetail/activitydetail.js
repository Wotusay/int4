    ...
    if (uiStore.currentBox === undefined) {
      if (!activity) {
        return (
          <>
            {" "}
            <Back /> <p className="loading">
              {" "}
              Activiteit is niet gevonden{" "}
            </p>{" "}
          </>
        );
      }
      ...
      return (
        <>
            ...
            >
              <div className={styles.contentleft}>
                <p className={styles.title}>
                  {" "}
                  Op bezoek in <span> {activity.city}!</span>
                </p>
                <div>
                  <p className={styles.subtitle}>{activity.title}</p>
                  <p className={styles.duration}>
                    Duurtijd: {activity.duration} mins{" "}
                  </p>
                </div>
                <p className={styles.desc}>{activity.description}</p>
                <p className={styles.info}>{activity.info}</p>
                <div className={styles.wrapper}>
                  <Link
                    to={
                      activity.title === "Raya-Yoga"
                        ? ROUTES.yogadetail
                        : activity.title === "Stargazing"
                        ? ROUTES.finland
                        : activity.title === "Buon Appetito"
                        ? ROUTES.pizzaconfigurator
                        : ROUTES.activities
                    }
          ...
          <div className={styles.relative}>
            <picture className={styles.right}>
              <source
                ...
                srcSet={`${activity.pic}.png`}
              />
              <source
                media="(min-width: 320px)"
                srcSet={`${activity.pic}-mobile.png`}
              />
              <img
                alt="india"
                style={{ width: "auto" }}
                src={`${activity.pic}.png`}
              />
            </picture>{" "}
          </div>
          ...
          <picture className={styles.left}>
            ...
            <source
              media="(min-width: 320px)"
              srcSet="/assets/leaf-top-mobile.png"
            />
            <img
              alt="prop-top"
              style={{ width: "auto" }}
